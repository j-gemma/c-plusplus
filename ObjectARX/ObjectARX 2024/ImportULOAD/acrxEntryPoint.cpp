...
public:
	...
	static void SFETest_ULOAD2() {
		...
		AcDbBlockTable* bt;
		if (db->getSymbolTable(bt, AcDb::kForWrite) != Acad::eOk) {
			acutPrintf(L"\nError opening block table");
			delete btr;
			return;
		}

		AcDbObjectId btrId;
		if (bt->add(btrId, btr) != Acad::eOk) {
			acutPrintf(L"\nError adding BTR.");
			bt->close();
			delete btr;
			return;
		...
		tbl->setSize(3, 3);
		tbl->setPosition(AcGePoint3d::kOrigin);
		tbl->setRowHeight(1.0);
		tbl->setColumnWidth(3.0);
		tbl->setTextString(0, 0, L"Header");

		//Populate more cells later, before generateLayout()

		//Add table to block definition
		AcDbObjectId tblId;
		if (btr->appendAcDbEntity(tblId, tbl) != Acad::eOk) {
		...
		AcDbBlockReference* bref =
			new AcDbBlockReference(AcGePoint3d(pt[0], pt[1], pt[2]), btrId);

		AcDbObjectId brefId;
		if (ms->appendAcDbEntity(brefId, bref) != Acad::eOk) {
			acutPrintf(L"\nInsert Failed.");
			delete bref;
			return;
		}
...
ACED_ARXCOMMAND_ENTRY_AUTO(CImportULOADApp, SFETest, _ULOAD, ULOAD, ACRX_CMD_MODAL, NULL)
ACED_ARXCOMMAND_ENTRY_AUTO(CImportULOADApp, SFETest, _ULOAD2, ULOAD2, ACRX_CMD_MODAL, NULL)
